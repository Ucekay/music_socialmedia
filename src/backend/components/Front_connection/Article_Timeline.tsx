...
import { getInitialArticleMetaData, getNewerMetaData, getOlderArticleMetaData } from "../DB_Access/Article/Article";
import { Database } from "../../types/supabasetypes";
import { getGeneral, GetGetArticleContent } from "../DB_Access/Article/General";
...
Promise<{articlemetaData: Articles[], cursor:string | null, latestcursor:string|null}>=> {
    let metadata;
...
export const getArticleContent = async (type:string, ArticleID:number):
        Promise<{Content:GetGetArticleContent, LiketoArticle:boolean}|null> =>{
        
        try{
            const userId = await checkAuth()
            let result: {Content:GetGetArticleContent, LiketoArticle:boolean}|null = null;

            switch (type) {
                case 'general':
                  const GeneralData = await getGeneral(ArticleID, userId) 
                  if(GeneralData !=null){
                    result = {Content: GeneralData.GeneralData, LiketoArticle: GeneralData.LikeToArticle}}
                  break;
                case 'review':
                  const ReviewData = await getReview(ArticleID, userId)
                  if(ReviewData !=null){
                    result = {Content: ReviewData?.ReviewData, LiketoArticle: ReviewData.LikeToArticle}}
                  break;
                case 'liveReport':
                  const LiveReportData = await getLiveReport(ArticleID, userId)
                  if(LiveReportData !=null){
                    result = {Content: LiveReportData?.LiveReportData, LiketoArticle: LiveReportData.LikeToArticle}}
                  break;
                case 'playlist':
                  const PlaylistData = await getPlaylistArticle(ArticleID, userId)
                  if(PlaylistData !=null){
                    result = {Content: PlaylistData.PlaylistArticleData, LiketoArticle: PlaylistData.LikeToArticle}}
                  break;
                default:
