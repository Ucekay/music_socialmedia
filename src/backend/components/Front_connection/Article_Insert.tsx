...
async (ArticleTypes: string[],  userId:string,thumbnailUrl: string, 
        type: string, title: string, info1: string|null, info2: string|null,
        body: string, playlistId?: number|null): 
        Promise<boolean | string> => {
  if (!ArticleTypes.includes(type)) {
    throw new Error('無効な記事タイプです。');
  }

  const ArticleMetaData = {ThumbnailURL: thumbnailUrl, Type: type,
    Title: title, Info1: info1, Info2: info2}
