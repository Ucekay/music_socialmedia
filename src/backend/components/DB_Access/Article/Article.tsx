...
type UpdateArticle = 
Omit<Database['public']['Tables']['Article']['Update'], 'ArticleID'| 'UserID'>
type GetArticle = Database['public']['Tables']['Article']['Row']

//データ挿入関数
export const insertArticleMetaData = async (
  Data: Omit<Article, 'ArticleID' | 'UserID' | 'crated_at'>): Promise<boolean> => {
    try{
...
//データ削除関数
export const deleteArticleMetaData = async (articleId: number): Promise<boolean> =>{
  try {
    const userId = await checkAuth();
    const {error} = await supabase
    .from('Article')
...
//データ更新関数
export const updateArticleMetaData = async (articleId: number, updateData:Partial<UpdateArticle>):
Promise<boolean> => {
  try {
    const userId = await checkAuth();
...
//データ取得関数
export const getInitialArticleMetaData = async (): Promise<{posts: GetArticle[], cursor:string|null ,latestcursor:string| null}> => {
    try {
  ...
  export const getOlderArticleMetaData =  async (cursor: string): Promise<{ posts: GetArticle[], cursor: string | null }> => {
    try{
    ...
    const nextCursor = nextArticles.length > 0 ? nextArticles[nextArticles.length - 1].created_at : cursor;
  ...
  export const getNewerMetaData = async (latestcursor: string): Promise<{ posts: GetArticle[], latestcursor:string| null }> => {
    try{
    ...
    const nextCursor = nextArticles.length > 0 ? nextArticles[0].created_at : latestcursor;
