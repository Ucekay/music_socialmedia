...
//データ取得関数
export const getInitialTodaysSongs = async (): Promise<{TodaysSongs: GetTodaysSong[], cursor:string|null ,latestcursor:string| null}> => {
  try {
    ...
    const cursor = TodaysSongs.length > 0 ? TodaysSongs[TodaysSongs.length - 1].created_at : null;
    const latestcursor = TodaysSongs.length> 0? TodaysSongs[0].created_at: null;
...
export const getOlderTodaysSongs =  async (cursor: string): Promise<{TodaysSongs: GetTodaysSong[], cursor: string | null }> => {
  try{
  ...
  const nextCursor = nextTodaysSongs.length > 0 ? nextTodaysSongs[nextTodaysSongs.length - 1].created_at : cursor;
...
export const getNewerTodaysSongs = async (latestcursor: string): Promise<{ TodaysSongs: GetTodaysSong[], latestcursor:string| null }> => {
  try{
  ...
  const nextCursor = nextTodaysSongs.length > 0 ? nextTodaysSongs[0].created_at : latestcursor;
